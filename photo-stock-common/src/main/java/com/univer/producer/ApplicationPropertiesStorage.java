...
public class ApplicationPropertiesStorage extends AbstractPropertiesLoader {
    ...
    private void postConstruct() {
        ...
        overrideApplicationProperties(applicationProperties, System.getenv(),
                "System environment");
    ...
    private void overrideApplicationProperties(Properties applicationProperties,
            Map<?, ?> map, String description) {
        String configFilePath = null;
        for (Map.Entry<?, ?> entry : map.entrySet()) {
            String key = String.valueOf(entry.getKey());
            if (key.startsWith(APPLICATION_CONFIG_PROPERTY_PREFFIX)) {
                applicationProperties.setProperty(key,
                        String.valueOf(entry.getValue()));
                logger.log(Level.INFO,
                        "Overrided application property {0}, defined in the {1}",
        ...
        if (configFilePath != null && Files.exists(Paths.get(configFilePath))) {
            ...
            logger.log(Level.INFO,
                    "Overrided application properties from file {0}, defined in the {1}",
